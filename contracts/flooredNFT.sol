...
contract FlooredApe is ERC721, ERC721URIStorage, Pausable, Ownable {
    ...
    function safeMint(address to, string memory uri) public whenNotPaused payable {
    ...
    function ownerMint(address to, string memory uri) public onlyOwner { //for airdrop
        _safeMint(to, _tokenIdCounter.current());
        _setTokenURI(_tokenIdCounter.current(), uri); 
        _tokenIdCounter.increment();
    }
